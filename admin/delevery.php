<?php
include 'php/config.php';
session_start();
if (!isset($_SESSION['access_token'])) {
    echo "<script>alert('Access Denied')</script>";
    echo "<script>alert('Redirect to Home Page')</script>";
    echo "<script>window.location.href = 'https://mypoetry.in';</script>";
    exit();
}
use PHPMailer\PHPMailer\PHPMailer;
use PHPMailer\PHPMailer\Exception;

require 'vendor/phpmailer/src/Exception.php';
require 'vendor/phpmailer/src/PHPMailer.php';
require 'vendor/phpmailer/src/SMTP.php';
function redirect($messageType)
{
    header("Location: delevery.php?message=$messageType");
    exit();
}
if (isset($_POST['sendmail-btn'])) {
    $user_id = $_POST['user_id'];
    $select = "SELECT `username`, `useremail` FROM `users` WHERE user_id=$user_id";
    $result_select = mysqli_query($con, $select);

    if ($result_select && $row = mysqli_fetch_assoc($result_select)) {
        $name = $row['username'];
        $email = $row['useremail'];


        $subject = "Your Payment is Pending";
        $message = "Please Complte Your Payment";
        $status_color = "orange";
        $status_text = "Pending";

        try {
            $mail = new PHPMailer(true);
            $mail->isSMTP();
            $mail->Host = 'mail.mypoetry.in';
            $mail->SMTPAuth = true;
            $mail->Username = 'admin@mypoetry.in';
            $mail->Password = 'Oigv4F24m6';
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;
            $mail->Port = 465;

            $mail->setFrom('admin@mypoetry.in', 'mypoetry.in');
            $mail->addAddress($email);

            // Send email
            $mail->addAddress($email);
            $mail->Subject = $subject;
            $mail->isHTML(true);
            $templatePath = "mail/payment.html";
            $templateContent = file_get_contents($templatePath);
            $templateContent = str_replace(["{username}", "{status-color}", "{status-text}", "{message}"], [$name, $status_color, $status_text, $message], $templateContent);
            $mail->Body = $templateContent;
            $mail->send();

            $_SESSION['message'] = ' Mail Sent';
            $messageType = "success";
        } catch (Exception $e) {
            $_SESSION['message'] = 'Failed to send mail';
            $messageType = "danger";
        }

    }

    redirect($messageType);
}


?>
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <meta http-equiv="X-UA-Compatible" content="ie=edge" />
    <?php include 'php/favicon.html' ?>
    <title>Delevery Page</title>
    <link href='https://unpkg.com/boxicons@2.1.4/css/boxicons.min.css' rel='stylesheet'>
    <link rel="stylesheet" href="styles/top3.css" />
    <link rel="stylesheet" href="styles/style.css" />
</head>

<body>

    <!-- controls -->
    <?php include 'php/slidebar.php'; ?>
    <?php
    if (isset($_SESSION['message'], $_GET['message'])):
        $messageClass = $_GET['message'];
        $messageText = $_SESSION['message'];
        ?>
        <div class="message" style="z-index:111px;">
            <div class="info <?= $messageClass ?>">
                <div class="info__icon">
                    <i class='bx bxs-info-circle'></i>
                </div>
                <div class="info__title">
                    <?= $messageText ?>
                </div>
                <div class="info__close" onclick="hideMessage()"><i class='bx bx-x-circle'></i></div>
            </div>
        </div>
    <?php endif; ?>
    <div class="content">
        <main>
            <div class="bottom-data">
                <div class="orders">
                    <div class="header">
                        <i class='bx bx-receipt'></i>
                        <h3>Poetry Delivery</h3>
                        <?php $query = "SELECT COUNT(*) as total_entries FROM `poetry_delivery`";
                        $total_rows = mysqli_query($con, $query);
                        $total_rows = mysqli_fetch_assoc($total_rows);
                        echo '<span> Total :' . $total_rows["total_entries"] . ' </span>'; ?>
                    </div>
                    <table>
                        <thead>
                            <tr>
                                <th>Person Name</th>
                                <th>Type</th>
                                <th>Language</th>
                                <th>Payment</th>
                                <th>Relation</th>
                                <th>Date</th>
                                <th>Image</th>
                                <th colspan="3">Status</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <?php
                                $query = "SELECT * FROM `poetry_delivery` ORDER BY 
                                CASE 
                                  WHEN payment_status = 'pending' THEN 1
                                  ELSE 0
                                END,
                                `id` DESC;
                              ";
                                $result = mysqli_query($con, $query);

                                // Check for errors in the query
                                if (!$result) {
                                    die("Query failed: " . mysqli_error($con));
                                }
                                while ($row = mysqli_fetch_assoc($result)) {
                                    ?>

                                    <td>
                                        <p>
                                            <?php echo $row['person_name']; ?>
                                        </p>
                                    </td>
                                    <td>
                                        <p>
                                            <?php echo $row['delevery_type']; ?>
                                        </p>
                                    </td>
                                    <td>
                                        <p>
                                            <?php echo $row['language']; ?>
                                        </p>
                                    </td>
                                    <td>
                                        <p>
                                            <?php echo $row['payment_status']; ?>
                                        </p>
                                    </td>
                                    <td>
                                        <p>
                                            <?php echo $row['relation_status']; ?>
                                        </p>
                                    </td>
                                    <td>
                                        <p title="<?php echo $row['date'] ?>">

                                            <?php $ratingDate = strtotime($row['date']);
                                            $currentDate = time();
                                            $differenceInSeconds = $currentDate - $ratingDate;
                                            $differenceInDays = floor($differenceInSeconds / (60 * 60 * 24));
                                            if ($differenceInDays == 0) {
                                                echo 'Today';
                                            } elseif ($differenceInDays == 1) {
                                                echo 'Yesterday';
                                            } elseif ($differenceInDays <= 60) {
                                                echo $differenceInDays . ' days ago';
                                            } else {
                                                $differenceInWeeks = ceil($differenceInDays / 7);
                                                echo $differenceInWeeks . ' weeks ago';
                                            } ?>
                                        </p>
                                    </td>
                                    <td>
                                        <?php 
                                       $person_img = $row['person_img'];
                                        $display_img = !empty($person_img) ? $person_img : 'source/Profile/defult.jpg'; ?>
                                        <img src="<?php echo $domain . '/' . $display_img ?>"
                                            alt="<?php echo $row['person_name']; ?>">
                                        </p>
                                    </td>
                                    <td class="d-flex">


                                        <?php if ($row['payment_status'] == "Pending") { ?>
                                            <form action="" method="post">
                                                <input type="hidden" value="<?php echo $row['id']; ?>" name="id">
                                                <input type="hidden" value="<?php echo $row['user_id']; ?>" name="user_id">
                                                <button type="submit" name="sendmail-btn" class="status process">Send
                                                    Mail</button>
                                            </form>
                                        <?php } ?>
                                        <?php if ($row['payment_status'] != "Pending") { ?>
                                            <form action="send-poetry.php" method="get">
                                                <input type="hidden" value="<?php echo $row['txt_id']; ?>" name="txt_id">
                                                <button type="submit" name="paid-btn" value="true"
                                                    class="status completed">Create</button>
                                            </form>
                                        <?php } ?>
                                </tr>
                                <?php
                                }
                                ?>
                        </tbody>
                    </table>
                </div>
            </div>
        </main>
    </div>
    <script src="scripts/index.js"></script>
    <script>
        function hideMessage() {
            window.location.href = "delevery.php";
        }

    </script>
</body>

</html>